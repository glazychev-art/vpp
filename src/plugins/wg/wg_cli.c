  ...
  pool_foreach (peer, peer_pool, (
                   {
                        ...
                        vlib_cli_output (vm, "Endpoint: %d.%d.%d.%d", peer->ip4_address.data[0], peer->ip4_address.data[1], peer->ip4_address.data[2] ,peer->ip4_address.data[3]);
                        vlib_cli_output (vm, "Allowed ip: %d.%d.%d.%d", peer->allowed_ip.data[0], peer->allowed_ip.data[1], peer->allowed_ip.data[2] ,peer->allowed_ip.data[3]);
                        vlib_cli_output (vm, "Tun_sw_if_index: %d", peer->tun_sw_if_index);
                        vlib_cli_output (vm, "Keepalive: %d", peer->persistent_keepalive_interval);
                   }
        ));
  vlib_cli_output (vm, "---------------------------------");
