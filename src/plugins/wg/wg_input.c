	...
	case MESSAGE_DATA:
	  {
		...
		0)
	      {
        next[0] = WG_INPUT_NEXT_IP4_LOOKUP;
        vnet_buffer (b[0])->sw_if_index[VLIB_RX] = peer->tun_sw_if_index;
	      }
	    break;
	  }
	default:
...
VLIB_REGISTER_NODE (wg_input_node) =
{
  ...
  .next_nodes = {
        [WG_INPUT_NEXT_IP4_LOOKUP] = "ip4-input-no-checksum",
